...
def conditionalStyle(condition, filters, columns):
    template = {
        'if': {
            'filter_query': filters[condition],
            'column_id': columns
        },
    }
    for k in conditionStyle.keys():
        template[k] = conditionStyle[k]
    return template
...
def conditionalStyles(conditionClass, columns):
    result = []
    for c in conditions:
        result.append(conditionalStyle(c, conditionClass, columns))
    return result
...
def make_style_data_conditional():
    ...
    result.append(conditionalStyles(FaellePro100kLetzte7TageClass, ['FaellePro100kLetzte7Tage']))
    result.append(conditionalStyles(AnzahlFallTrendClass, ['AnzahlFallTrend']))
    result.append(conditionalStyles(KontaktrisikoClass, ['Kontaktrisiko']))
    return result
...
h_header = html.Header(
    ...
    children=[
        ...
        html.H4(className="app-header-date",
                children="Softwarestand: {} (UTC), Version 0.9.3".format(appDateStr),
                style={'color': colors['text']})
    ]
)
